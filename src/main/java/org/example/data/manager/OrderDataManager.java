...
public class OrderDataManager extends BaseDataManager<Order> {
    ...
    private static Order mapToOrderAddNew(String[] data) {
        try {
            Order order = new Order();
            String orderId = data[CSVColumn.OrderColumn.ID.getIndex()];
            order.setId(orderId != null && !orderId.isEmpty() ? orderId : generateOrderId());

            order.setCustomerId(data[CSVColumn.OrderColumn.CUSTOMER_ID.getIndex()]);
    ...
    // Phương thức sinh ID đơn hàng
    private static String generateOrderId() {
        // Logic để sinh ra một ID duy nhất cho đơn hàng mới
        // Có thể dùng UUID hoặc một cách khác tùy ý bạn
